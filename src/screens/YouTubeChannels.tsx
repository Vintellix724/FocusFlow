...
export default function YouTubeChannels() {
  const { youtubeChannels, addYouTubeChannel, deleteYouTubeChannel, theme, toggleTheme } = useStore();
  ...
  const handleAdd = (e: React.FormEvent) => {
    ...
    addYouTubeChannel({ name: name.trim(), url: finalUrl });
    setName('');
    setUrl('');
  };
    ...
    <div className="min-h-screen bg-surface pb-24">
      ...
      <header className="bg-surface-container-highest pt-12 pb-6 px-6 rounded-b-[2rem] shadow-sm relative z-10">
        <div className="flex justify-between items-center mb-4">
          <h1 className="font-syne font-bold text-3xl text-on-surface">YouTube Channels</h1>
          <button 
            onClick={toggleTheme}
        ...
        <p className="font-body text-on-surface-variant">Manage your favorite study channels.</p>
      </header>
        ...
        <div className="space-y-4">
          {youtubeChannels.length === 0 ? (
            <div className="text-center py-12 bg-surface-container rounded-2xl border border-outline-variant/20 border-dashed">
              <span className="material-symbols-outlined text-4xl text-on-surface-variant/50 mb-2">smart_display</span>
